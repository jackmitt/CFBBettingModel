...
z = []
allScores = []
a = pd.read_csv('./new_csv_Data/bigboyBinClassificationTrain.csv', encoding = "ISO-8859-1")
y_train = a["binSpread"]
xCols = []
...
bSpread = pd.read_csv('./new_csv_Data/bigboyBinClassificationTest.csv', encoding = "ISO-8859-1")
...
z = []
allScores = []
a = pd.read_csv('./new_csv_Data/bigboyBinClassificationTotalsTrain.csv', encoding = "ISO-8859-1")
y_train = a["binTotal"]
xCols = []
for col in a.columns:
    if ("aboveAvg" in col):
...
X_train = pd.DataFrame(a, columns = xCols)
X_train[xCols] = scaler.fit_transform(X_train[xCols])
bTotal = pd.read_csv('./new_csv_Data/bigboyBinClassificationTotalsTest.csv', encoding = "ISO-8859-1")
y_test = bTotal["binTotal"]
X_test = pd.DataFrame(bTotal, columns = xCols)
X_test[xCols] = scaler.transform(X_test[xCols])

model = LogisticRegression(max_iter = 100000, C = 0.001)
model.fit(X = X_train, y = y_train)
for p in model.predict_proba(X_test):
    if (model.classes_[1] == 1):
        predictions.append(p[1])
...
# dfFinal.to_csv("./csv_Data/betsForOptimization.csv")
