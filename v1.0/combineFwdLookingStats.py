...
for year in years:
    stats = pd.read_csv('./new_csv_Data/advStatsFwdLooking/' + str(year) + '.csv', encoding = "ISO-8859-1").drop(columns = ["Unnamed: 0"])
    adjStats = pd.read_csv('./new_csv_Data/adjAdvStatsFwdLooking/' + str(year) + '.csv', encoding = "ISO-8859-1").drop(columns = ["Unnamed: 0"])
    ...
    final.to_csv("./new_csv_Data/combinedFwdLookingStats/" + str(year) + ".csv")
