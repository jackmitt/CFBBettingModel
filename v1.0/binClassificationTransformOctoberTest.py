...
for w in we:
        ...
        for col in a.columns:
            if ('H' == col[0] and 'o' == col[1] and "Score" not in col):
                if (homeFav):
                    ...
                    dict["FE_" + col.split("Ho")[1]].append((float(row[col]) + float(row["Rd" + col.split("Ho")[1]]) - a[col].mean()))
                else:
                    if ("DE_" + col.split("Ho")[1] not in dict):
                        dict["DE_" + col.split("Ho")[1]] = []
                    dict["DE_" + col.split("Ho")[1]].append((float(row[col]) + float(row["Rd" + col.split("Ho")[1]]))  - a[col].mean())
            elif ('R' == col[0] and 'o' == col[1] and "Score" not in col):
                if (homeFav):
                    if ("DE_" + col.split("Ro")[1] not in dict):
                        dict["DE_" + col.split("Ro")[1]] = []
                    dict["DE_" + col.split("Ro")[1]].append((float(row[col]) + float(row["Hd" + col.split("Ro")[1]]))  - a[col].mean())
                else:
                    if ("FE_" + col.split("Ro")[1] not in dict):
                        dict["FE_" + col.split("Ro")[1]] = []
                    dict["FE_" + col.split("Ro")[1]].append((float(row[col]) + float(row["Hd" + col.split("Ro")[1]]))  - a[col].mean())
            elif ("Hadj_offense." in col):
                if (homeFav):
                    if ("FE_adj_" + col.split("Hadj_offense.")[1] not in dict):
                        dict["FE_adj_" + col.split("Hadj_offense.")[1]] = []
                    dict["FE_adj_" + col.split("Hadj_offense.")[1]].append((float(row[col]) + float(row["Radj_defense." + col.split("Hadj_offense.")[1]])) - a[col].mean())
                else:
                    if ("DE_adj_" + col.split("Hadj_offense.")[1] not in dict):
                        dict["DE_adj_" + col.split("Hadj_offense.")[1]] = []
                    dict["DE_adj_" + col.split("Hadj_offense.")[1]].append((float(row[col]) + float(row["Radj_defense." + col.split("Hadj_offense.")[1]])) - a[col].mean())
            elif ("Radj_offense." in col):
                if (homeFav):
                    if ("DE_adj_" + col.split("Radj_offense.")[1] not in dict):
                        dict["DE_adj_" + col.split("Radj_offense.")[1]] = []
                    dict["DE_adj_" + col.split("Radj_offense.")[1]].append((float(row[col]) + float(row["Hadj_defense." + col.split("Radj_offense.")[1]])) - a[col].mean())
                else:
                    if ("FE_adj_" + col.split("Radj_offense.")[1] not in dict):
                        dict["FE_adj_" + col.split("Radj_offense.")[1]] = []
                    dict["FE_adj_" + col.split("Radj_offense.")[1]].append((float(row[col]) + float(row["Hadj_defense." + col.split("Radj_offense.")[1]])) - a[col].mean())
    ...
    for key in dict:
        a[key] = dict[key]
    a = a.dropna()
    a.to_csv("./new_csv_Data/bigboyBinClassificationOctTest.csv")
