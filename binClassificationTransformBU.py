...
for w in we:
    ...
    for index, row in a.iterrows():
        ...
        if (getBin(row["alt_spread"], W234 = True) != -1 and not np.isnan(row["binSpread"])):
            if (getBin(row["alt_spread"], W234 = True) not in reg):
                reg[getBin(row["alt_spread"], W234 = True)] = {"Count":0}
            reg[getBin(row["alt_spread"], W234 = True)]["Count"] += 1
    print (reg)
    print ("---------------------------------------------------")
    print (len(a.index))
    for key in dict:
    ...
    for key in dict:
        a[key] = dict[key]
    for index, row in a.iterrows():
        if (getBin(row["Spread"], W234 = True) == -1):
            continue
        for col in a.columns:
            if ("_diff" in col or col == "FavHF"):
                if (col not in reg[getBin(row["Spread"], W234 = True)]):
                    reg[getBin(row["Spread"], W234 = True)][col] = []
                reg[getBin(row["Spread"], W234 = True)][col].append(float(row[col]))
    dict = {}
    temp = []
    for index, row in a.iterrows():
        for col in a.columns:
            if ("_diff" in col or col == "FavHF"):
                ...
                if (getBin(row["Spread"], W234 = True) == -1):
                    ...
                    continue
                temp = reg[getBin(row["Spread"], W234 = True)][col].copy()
                try:
